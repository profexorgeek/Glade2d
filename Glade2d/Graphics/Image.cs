...
namespace Glade2d.Graphics
{
    ...
    public partial class Image
    {
        ...
        private void LoadBitmap(Stream source)
        {
            ...
            LogService.Log.Trace($"Reading bitmap data from offset {dataOffset}.");
            // HACK: change the data offset to see what happens
            //dataOffset += 256;
            ...
            LogService.Log.Trace($"Got bitmap dimensions of {width}x{height}");
            ...
            LogService.Log.Trace($"Got data size {dataSize} with compression type {compression} and depth of {BitsPerPixel}.");
            ...
            LogService.Log.Trace($"Got bytes per row of {bytesPerRow} with needed padding of: {rowPad}.");
        ...
        private void ConvertRGBBufferToBGRBuffer(byte[] buffer)
        {
            ...
            LogService.Log.Trace("Converting RGB Buffer to BGR Buffer...");
